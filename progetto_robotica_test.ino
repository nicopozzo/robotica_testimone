...
#define IN_CHARACTERISTIC_UUID "41086b21-669b-4bfc-8ac5-cc7a765c5882" //IN: cell --> esp32
#define OUT_CHARACTERISTIC_UUID "46cca60d-a0e1-4518-8baa-a9e075068d66" //OUT: esp32 --> cell
...
BLECharacteristic *p_in_Characteristic;
BLECharacteristic *p_out_Characteristic;
...
void BLE_setup(){
 ...
 /* p_to_be_mimed_Characteristic = pService->createCharacteristic(
  ...
  p_in_Characteristic = pService->createCharacteristic(
                                         IN_CHARACTERISTIC_UUID,
                                         BLECharacteristic::PROPERTY_READ |
                                         BLECharacteristic::PROPERTY_WRITE
                                       );  
  p_out_Characteristic = pService->createCharacteristic(
  ...
  p_in_Characteristic->setValue("def");
  p_out_Characteristic->setValue("def");
...
void addEmotion(int emotion){
  ...
  if(inserted==0){
    p_out_Characteristic->setValue("Coda piena. Impossibile impostare più di 20 emozioni");
  }
}
...
void BLE_read(){
  ...
  //==== SET_MODE
  std::string cmdIn = p_in_Characteristic->getValue();
  ...
  //==== TO_BE_MIMED
  cmdIn = p_in_Characteristic->getValue();
  ...
  if (startButtState == HIGH) {
    ...
    if(startPressing == 0){
      ...
      if((millis()-startPressing)/1000 >= 5){
        state = "T0";
        p_out_Characteristic->setValue("Modalità allenamento impostata");
    ...
    if(pressedStart==0){
      ...
      if(state == "F0"){ //stato iniziale della fun_mode. al momento se viene pigiato start allo stato F1 non succede nulla (il testimone è in attesa della risposta)
        ...
        emotion = getEmotion();
        p_out_Characteristic->setValue(emotion); //COMUNICAZIONE BT ALL'APP
      ...
      if(state.equals("T1") && switchedState == 0){
        ...
        p_out_Characteristic->setValue(elapsedSec);//TODO ripristanare a "def" quando letto da app
        Serial.println("elapsed secs sent over BT");
        std::string thresh_in = p_in_Characteristic->getValue();
        ...
        std::string cust_fb = p_in_Characteristic->getValue();
        p_in_Characteristic->setValue("def");
  ...
  for(int i=0;i<4;i++){
    ...
    if (emotionButtState == HIGH && pressedButtPins[i] == 0) {
      ...
      if(state == "F1"){
        ...
        p_out_Characteristic->setValue(request[i].c_str()); //da ripristinare da app a "def" una volta ricevuto
